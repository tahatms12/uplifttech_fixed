...
- The function validates the Origin header against `TRAINING_APP_ORIGIN` (plus defaults) and compares the submitted password to the server-only `DEMO_KEY`/`demo_key`. Successful logins mint an HTTP-only cookie signed with `TRAINING_JWT_SECRET`. 【F:netlify/functions/training-api.ts†L24-L86】【F:netlify/functions/_lib/security.ts†L63-L84】
...
| `DEMO_KEY` (or `demo_key`) | `netlify/functions/training-api.ts` | Function runtime | The password compared during login; must match what users enter. | None – missing returns 500 on login. | Secret | All deploy contexts |
...
1. In Netlify **Site configuration → Environment variables**, add (or verify) these key/value pairs for **Production**, **Deploy Previews**, and **Branch Deploys**:
   ...
   - `DEMO_KEY` = `[REDACTED]` (the password users must enter)
...
3. Visit `http://localhost:8888/training`, submit the email/password (matching `DEMO_KEY`), and confirm redirect to `/training/dashboard`. Subsequent requests to `/api/training/auth/me` should return 200 with user info.
...
2. Log in with an email and the configured `DEMO_KEY` password.
...
- **500 on login**: `DEMO_KEY`/`demo_key` missing; add it to environment variables and redeploy.
