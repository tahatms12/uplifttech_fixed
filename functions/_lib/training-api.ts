...
import { jsonResponse, methodNotAllowed, notFound, parseJsonBody, serverError } from './http';
import { getAdminEmails, getEnv } from './env';
...
import { courseTitle, getQuiz, listCatalogCourses } from './catalog';
import { buildProgress, getCourseProgress, loadTrainingData, type CourseProgressSummary } from './completion';
...
function resolveAllowedOrigins(event: HandlerEvent): string[] {
  const configured = (getEnv(event.env, 'TRAINING_APP_ORIGIN') || '')
    .split(',')
    .map((origin) => origin.trim())
    .filter(Boolean);
  const defaults = [
    event.env?.CF_PAGES_URL,
...
function loadConfig(event: HandlerEvent): TrainingConfig {
  if (cachedConfig) return cachedConfig;
  const jwtSecret = getEnv(event.env, 'TRAINING_JWT_SECRET') as string | undefined;
  const demoKey = (getEnv(event.env, 'demo_key') || getEnv(event.env, 'DEMO_KEY')) as string | undefined;
  const demoUsername = getEnv(event.env, 'DEMO_USERNAME') as string | undefined;
  const cookieName = getEnv(event.env, 'TRAINING_COOKIE_NAME', false, 'training_session') as string;
  const trainingOrigins = resolveAllowedOrigins(event);
  const adminEmails = getAdminEmails(event.env);
...
async function parseAuth(event: HandlerEvent, config: TrainingConfig): Promise<AuthenticatedUser | null> {
...
async function handleLogin(event: HandlerEvent, config: TrainingConfig): Promise<HandlerResponse> {
  ...
  const ipHash = await hashIp(ip, config.jwtSecret);
...
async function handleMe(event: HandlerEvent, config: TrainingConfig): Promise<HandlerResponse> {
  const auth = await parseAuth(event, config);
  if (!auth) {
...
): Promise<{ auth: AuthenticatedUser | null; error?: HandlerResponse }> {
  const auth = await parseAuth(event, config);
  if (!auth) {
...
async function handleAdminUsers(event: HandlerEvent, config: TrainingConfig): Promise<HandlerResponse> {
  const { error } = await requireAdmin(event, config);
...
async function handleCertificateIssue(event: HandlerEvent, config: TrainingConfig): Promise<HandlerResponse> {
  ...
  const certificateCode = randomHex(16);
...
export async function handleTrainingRequest(event: HandlerEvent): Promise<HandlerResponse> {
  try {
